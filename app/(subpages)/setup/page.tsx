		...
		<Card className="flex flex-col">
			<header className="p-6 flex flex-col">
				...
				<p className="text-black-10 dark:text-white-10">
					Este é meu humilde setup, onde dou vida às minhas ideias. Nesta
					página, vou compartilhar com vocês cada periférico que uso no meu dia
					a dia e o link de onde comprei cada um deles.
				</p>
			</header>
			...
			<div className="flex w-full overflow-x-auto">
				<table className="table-fixed divide-y min-w-full">
					<caption className="caption-bottom border-t text-sm text-black-10 dark:text-white-10 p-2">
						Os links nessa tabela não estão necessariamente com os melhores
						preços.
					</caption>
					<thead>
						<tr>
					...
					<tbody className="divide-y">
						{SETUP_ITEMS.map((item, index) => {
							return (
								<tr
									key={item.product}
									className="hover:bg-black-1 dark:hover:bg-white-1"
								>
									...
									<td className="px-6 py-4 align-text-top">
										{item.purchaseLink && (
											<ExternalLink href={item.purchaseLink}>
												Comprar
											</ExternalLink>
										)}
									</td>
